%     Script file: blankstim
%
%     Substitutes NaN values between lower and upper x-limits defined
%     graphically by left-clicking the mouse cursor then right-clicking
%     or pressing the 'RETURN' key. The resulting data matrix is exported
%     as an ascii text tile (.txt) with the filename ending '_cp' if it
%     does not exist already.
%
%     blankstim v1.1 (last updated: 22/02/2017)
 ...
 [gx, gy, button]=ginput(1);
  ...
  if exist('gX') == 1
   if length(gX) == 2 && button ~=3 && button ~=13
    ...
    clf
    plot(tl,yl,'k-');grid('off');xlim([min(tl),max(tl)]); xlim([gX(1),gX(2)]); ylim([y_minlim y_maxlim]); box('off');
   end
  end
end

% Substitute NaN values between coordinates and replot data
output_data=[tl yl];
if exist('limits')
  for i=1:n
   output_data(limits(1):limits(2),i+1)=NaN;
  end
end
clf(1);
